...
assert eos_def is not None # having these asserts is kinda not elegant, but for i havent found a prettier way 
...
logRho = np.log10(1.538306628e+05*c.cm*c.cm*c.cm/c.gram)  # 9.0  # log10 of density in cgs
logT = np.log10(1.566687888e+07)     # 9.0  # log10 of temperature
lnfree_e = (
    0.0  # free_e := total combined number per nucleon of free electrons and positrons
)
...
massFractions = (
    {  # these are user friendly input, feel free to change them # TODO TEST THIS
...
massFractionsInArr = np.array([], dtype=float)  # these are the mass fractions we use
chem_id = np.array([], dtype=int)  # these are their chemical ids
net_iso = np.zeros(num_chem_isos, dtype=int)  # maps chem id to species number (index in the array I suppose? idk man, mesa ppl rly dont like clarity)
