cc     Calling propram should contain something like following lines.
cc     See subroutine esac for definitions
...
cc                  in esac.
cc     NOTE: irad=0 does not add radiation; irad=1 adds radiation
...
c      endif
c      write (*,*) r
c      call esac(x,ztab,t6,r,iorder,irad,reteos) ! calc EOS; use r from rhoofp
c      write (*,'("eos=",9e14.4)') (eos(i),i=1,9)
cc                                            if fixedTP='yes'
c      end


c*************************************************************************
      subroutine esac (xh,ztab,t6,r,iorder,irad, reteos)
...
c            eos(2) is energy in 10**12 ergs/gm. Zero is zero T6   erg=1e-7J
...
c..... The purpose of this subroutine is to read the data tables
     ...
     x    rhogr(mx,nr),frac(mx,6),alogr(nr,nt)
      ...
      close (2)
c..... read  tables
      call system ('gunzip EOSdata.gz')
      open(2, FILE='OPAL/EOSdata/EOSdata')
     ...
     x      e14.4)') xin(m), rhogr(m,jcs)
      ...
      read (2,'(f9.5,1x,f6.2,3e13.5,6f8.4)')
...
c      pnr = 1.133566618-1
      ...
      if (itime .ne. 12345678) then
      call esac (0.5,ztab,1.,0.001,1,0,reteos) ! x 0.5 nahrazeno 0.7
      endif
...
c      if((pnr .gt. 1.25*pmax) .or. (pnr .lt. pmin)) then
c      write (*,'(" The requested pressure-temperature not in table")')
cc     stop
c      write (*,'("pnr, pmax,pmin=",3e14.4)') pnr,pmax,pmin
c      endif
      
      rhog1=rho(nra(klo))*pnr/pmax
      call esac (x,ztab,t6,rhog1,1,0,reteos)
      p1=eos(1)
        if(p1 .gt. pnr) then
          p2=p1
          rhog2=rhog1
...
c          rhog1=9.e-10
          ...
          call esac (x,ztab,t6,rhog2,1,0,reteos)
          p2=eos(1)
        endif
   ...
   1  icount=icount+1
      ...
      call esac (x,ztab,t6,rhog3,1,0,reteos)
c      call esac (0.7, 0.02, 1., rhog3, 1, 0, reteos)
      p3=eos(1)
