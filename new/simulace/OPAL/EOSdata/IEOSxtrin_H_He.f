...
c        ztab is metal fraction of the EOSdata_H-He tables you are using.
...
c..... read the data files
        ...
        write (*,'("requested z=",f10.6," EOSdata_H-He is for z=",
     x             f10.6)')
     x    ztab,z      
        stop
        endif
...
c__________
  ...
  123 continue
      if((zz(mg) .ne. zz(mf)) .or. (zz(mh) .ne. zz(mf))) then
        write(*,'("Z does not match Z in EOSdata_H-He files you are"
     x ," using")')
        stop
      endif
...
c
  ...
  124 continue
   ...
   65 write(*,'("T6/log rho in empty region od table (65)")')
      ...
      stop
   66 write(*,'(" Z does not match Z in EOSdata_H-He* files you are",
     . " using (66)")')
...
c..... The purpose of this subroutine is to read the data tables
     ...
     x    rhogr(mx,nr),frac(mx,6),alogr(nr,nt)
      ...
      close (2)
c..... read  tables
       call system (' gunzip EOSdata_H-He')
       open(2, FILE='EOSdata_H-He')
